//@ sourceURL=script/lib/table/GridManager.js
          ...
          */
            value: function createDOM($table) {
                ...
                if(settings.supportMenu){
                    wrapHtml += '<div class="CustomFilterButton"><button style="background:#1984B6;color:#fff;border:1px solid #1984B6;margin:4px;" class="tableFilterBtn">自定义字段</button><button style="background:#8BC34A;color:#fff;border:1px solid #8BC34A;margin:4px;border-radius:5px;display:none;" class="tableFilterAllBtn">全选</button><button style="background:#F44336;color:#fff;border:1px solid #F44336;margin:4px;border-radius:5px;display:none;" class="tableFilterNoBtn">清空</button></div>';
                }
                wrapHtml += '<div class="table-div" style="height:calc(' + settings.height + ' - 40px)"></div>\n\t\t\t\t\t\t<span class="text-dreamland"></span>\n\t\t\t\t\t</div>';
                $table.wrap(wrapHtml);
...
/***/ (function (module, exports, __webpack_require__) {
    ...
    //var _MenuClick = __webpack_require__(24);     //自定义修改

    //var _MenuClick2 = _interopRequireDefault(_MenuClick);     //自定义修改
       ...
       * @param settings
          */
            ...
            value: function initTable($table, settings) {
                ...
                // 绑定表头menu点击事件     自定义修改
                //_MenuClick2.default.bindMenuClickEvent($table);
            ...
            value: function bindConfigEvent($table) {
                ...
                configAction.bind('click', function () {
                    ...
                    if (_configArea.css('display') === 'block') {
                        _configArea.hide();
                        if ((0, _Base.$)('.tableFilterBtn', tableWarp).html() == '返回') {
                            (0, _Base.$)('.tableFilterAllBtn', tableWarp).hide();
                            (0, _Base.$)('.tableFilterNoBtn', tableWarp).hide();
                            (0, _Base.$)('.tableFilterBtn', tableWarp).html('自定义字段');
                        }
                        return false;
                    }
                ...
                (0, _Base.$)('.config-list li', tableWarp).bind('click', function () {
                    ...
                    _Base.$.each(_visibleTh, function (i, v) {
                        ...
                        if (widthLidt[(0, _Base.$)(v).attr('th-name')]) {
                            var _settingsWidth = widthLidt[(0, _Base.$)(v).attr('th-name')].split('px')[0];
                            if (_thWidth < _settingsWidth) {
                                (0, _Base.$)(v).width(_settingsWidth);
                            } else {
       ...
       * @param $table
          */
            ...
            value: function bindRightMenuEvent($table) {
                ...
                // 刷新当前表格
                var menuHTML = '<div class="grid-menu" grid-type="' + Settings.timeStamp + '" grid-master="' + Settings.gridManagerName + '">';
                ...
                tableWarp.off('click', '.tableFilterBtn')
                tableWarp.on('click', '.tableFilterBtn', function (e) {
                    //debugger;
                    if (_this.isDisabled(this, e)) {
                        return false;
                    }
                    var _gridMenu = (0, _Base.$)('.grid-menu[grid-type="' + Settings.timeStamp+ '"]');
                    var _table = (0, _Base.$)('table[grid-manager="' + _gridMenu.attr('grid-master') + '"]', tableWarp);
                    var configArea = (0, _Base.$)('.config-area', _table.closest('.table-wrap'));
                    (0, _Base.$)('.config-action', configArea).trigger('click');
                    _body.off('mousedown.gridMenu');
                    _gridMenu.hide();
                    if ((0, _Base.$)('.config-list', tableWarp).parent().attr('style') == 'display: block;') {
                        (0, _Base.$)(this).html('返回');
                        (0, _Base.$)('.tableFilterAllBtn', tableWarp).show();
                        (0, _Base.$)('.tableFilterNoBtn', tableWarp).show();
                    } else {
                        (0, _Base.$)(this).html('自定义字段');
                        (0, _Base.$)('.tableFilterAllBtn', tableWarp).hide();
                        (0, _Base.$)('.tableFilterNoBtn', tableWarp).hide();
                    }
                })
                tableWarp.off('click', '.tableFilterAllBtn')
                tableWarp.on('click', '.tableFilterAllBtn', function (e) {

                    if (_this.isDisabled(this, e)) {
                        return false;
                    }
                    _Base.$.each((0, _Base.$)('.config-list li', tableWarp), function (k, item) {
                        if (!(0, _Base.$)(item).hasClass('checked-li') && (0, _Base.$)(item).attr('style') != 'display: none;') {
                            (0, _Base.$)(item).trigger('click')
                        }
                    })
                })
                tableWarp.off('click', '.tableFilterAllBtn')
                ...
                tableWarp.off('click', '.tableFilterNoBtn')
                tableWarp.on('click', '.tableFilterNoBtn', function (e) {

                    if (_this.isDisabled(this, e)) {
                        return false;
                    }
                    _Base.$.each((0, _Base.$)('.config-list li'),function (k, item) {
                        if ((0, _Base.$)(item).hasClass('checked-li')) {
                            if ((0, _Base.$)(item).text().indexOf('序号') == -1) {
                                (0, _Base.$)(item).trigger('click')
                            } else {
...
/***/ (function (module, exports, __webpack_require__) {
    ...
    var Settings = function Settings() {
      ...
      */
        ...
        var settings = _extends({}, drag, adjust, remind, config, gridStyle, cache, sort, ajaxPage, autoOrder, checkbox, i18n, gridData, gridExport, timeStamp);
...
//        function MenuClick() {
//            _classCallCheck(this, MenuClick);
//        }

//        _createClass(MenuClick, [{
//            key: 'bindMenuClickEvent',

//            /**
//       * 绑定拖拽换位事件
//       * @param $table
//       */
//            value: function bindMenuClickEvent($table) {
//                var _this = this;
//                var $tableBox = $table.closest('.table-wrap')
//                // 指定拖拽换位事件源,配置拖拽样式
//                $tableBox.off('click', '.tableFilterBtn');
//                $tableBox.on('click', '.tableFilterBtn', function (event) {
//                    (0, _Base.jTool)('[grid-action="config-grid"]').trigger('click')
//                    (0, _Base.jTool)('[grid-action="config-grid"]').trigger('click')
//                    // $tableBox.find('[grid-action="config-grid"]').trigger('click')
//                    if ((0, _Base.jTool)('.config-list', $tableBox).parent().attr('style') == 'display: block;') {
//                        // if ($tableBox.find('.config-list').parent().attr('style') == 'display: block;') {
//                        $tableBox.find('.tableFilterAllBtn').show();
//                        $tableBox.find('.tableFilterNoBtn').show();
//                        // if (Map.realSelectTable != Map.selectTable) {
//                        // $tableBox.find('#tableFilterAllBtn').trigger('click')
//                        // Map.realSelectTable = Map.selectTable
//                        // }
//                    } else {
//                        (0, _Base.jTool)('.tableFilterAllBtn', $tableBox).hide()
//                        (0, _Base.jTool)('.tableFilterNoBtn', $tableBox).hide()
//                        // $tableBox.find('.tableFilterAllBtn').hide();
//                        // $tableBox.find('.tableFilterNoBtn').hide();
//                    }
//                });
//                $tableBox.off('click', '.config-action')
//                $tableBox.on('click', '.config-action', function () {
//                    // $('.ajax-content #tableFilterAllBtn').hide();
//                    // $('.ajax-content #tableFilterNoBtn').hide();
//                    (0, _Base.jTool)('.tableFilterAllBtn', $tableBox).hide()
//                        (0, _Base.jTool)('.tableFilterNoBtn', $tableBox).hide()

//                })
//                $tableBox.off('click', '.tableFilterAllBtn');
//                $tableBox.on('click', '.tableFilterAllBtn', function (event) {
//                    (0, _Base.jTool)('.config-list li', $tableBox).each(function (k, item) {
//                        if (!$(item).hasClass('checked-li') && $(item).attr('style') != 'display: none;') {
//                            $(item).trigger('click')
//                        }
//                    })
//                });
...
//                $tableBox.on('click', '.tableFilterBtn', function (event) {
//                    (0, _Base.jTool)('.config-list li', $tableBox).each(function (k, item) {
//                        if ((0, _Base.jTool)(item).hasClass('checked-li')) {
//                            if ((0, _Base.jTool)(item).children('label').text().indexOf('序号') == -1) {
//                                (0, _Base.jTool)(item).trigger('click')
//                            } else {
